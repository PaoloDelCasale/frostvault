"""Durable catalog revisions/events and bounded filesystem health persistence.
...
and event insert commit or roll back together.  Filesystem health snapshots are
also derived cache data: they are versioned by the catalog revision and never
replace the canonical Vault File tables.
"""
...
MAX_HEALTH_SUMMARY_BYTES = 16384
...
class FilesystemHealthStore:
    """Persist one bounded, revision-keyed filesystem health snapshot."""

    def __init__(self, connection: Any):
        self.connection = connection

    @staticmethod
    def _finding_value(finding: Any, name: str, default: str = "") -> Any:
    ...
    def _normalize_finding(cls, finding: Any) -> dict[str, Any]:
        code = _bounded_text(
            cls._finding_value(finding, "code"),
            name="finding code",
            maximum=MAX_HEALTH_CODE_LENGTH,
            allow_empty=False,
        )
        if _CODE_RE.fullmatch(code) is None:
            raise ValueError("finding code contains unsupported characters")
        scope = _bounded_text(
            cls._finding_value(finding, "scope"),
            name="finding scope",
            maximum=MAX_HEALTH_SCOPE_LENGTH,
        )
        path = _bounded_text(
            cls._finding_value(finding, "path"),
            name="finding path",
            maximum=MAX_HEALTH_PATH_LENGTH,
        )
        if path.startswith(("/", "\\")) or any(
            segment == ".." for segment in path.split("/")
        ):
            raise ValueError("finding path must be relative to the Vault root")
        message = _bounded_text(
            cls._finding_value(finding, "message"),
            name="finding message",
            maximum=MAX_HEALTH_MESSAGE_LENGTH,
            allow_empty=False,
        )
        remediation = _bounded_text(
            cls._finding_value(finding, "remediation"),
            name="finding remediation",
            maximum=MAX_HEALTH_REMEDIATION_LENGTH,
        )
        occurrences = cls._finding_value(finding, "occurrences", 1)
        if isinstance(occurrences, bool) or not isinstance(occurrences, int) or occurrences < 1:
            raise ValueError("finding occurrences must be a positive integer")
        return {
            "code": code,
            "scope": scope,
            "path": path,
    ...
    def save_snapshot(
        ...
        *,
        ...
        vault_id = _positive_or_zero(vault_id, name="vault_id")
        catalog_revision = _positive_or_zero(
            catalog_revision,
            name="catalog_revision",
        )
        if vault_id == 0:
            raise ValueError("vault_id must be positive")
        if status not in {"checking", "current", "stale", "failed"}:
            raise ValueError("unsupported filesystem health status")
        normalized_findings: list[dict[str, Any]] = []
        for finding in findings:
            if len(normalized_findings) >= MAX_HEALTH_FINDINGS_PER_SNAPSHOT:
                raise ValueError("filesystem health findings exceed the bounded sample")
            normalized_findings.append(self._normalize_finding(finding))
        if total_findings is None:
            total = len(normalized_findings)
        else:
            total = _positive_or_zero(total_findings, name="total_findings")
        if total < len(normalized_findings):
            raise ValueError("total_findings cannot be below sampled findings")
        sampled = len(normalized_findings)
        summary_json = _json_object(
            summary,
            name="filesystem health summary",
            maximum=MAX_HEALTH_SUMMARY_BYTES,
        )
        if uid is not None:
            uid = _positive_or_zero(uid, name="uid")
        if gid is not None:
            gid = _positive_or_zero(gid, name="gid")
        normalized_error_code = (
            _bounded_text(
                error_code,
                name="health error code",
                maximum=MAX_HEALTH_ERROR_CODE_LENGTH,
            )
        ...
        normalized_error_message = (
            _bounded_text(
                error_message,
                name="health error message",
                maximum=MAX_HEALTH_ERROR_LENGTH,
            )
        ...
        self.connection.execute(
            """
            INSERT INTO filesystem_health_snapshots(
                id, vault_id, catalog_revision, status, created_at, checked_at,
                total_findings, sampled_findings, findings_truncated, summary_json,
                uid, gid, error_code, error_message
            ) VALUES (
                %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
                %s, %s, %s, %s
            )
        ...
        for finding in normalized_findings:
            self.connection.execute(
                """
                INSERT INTO filesystem_health_findings(
                    snapshot_id, code, scope, path, message, remediation, occurrences
        ...
        return {
            ...
            "findings_truncated": sampled < total,
            "summary": _decode_object(summary_json, name="filesystem health summary"),
            "uid": uid,
            "gid": gid,
    ...
    record_snapshot = save_snapshot

    @staticmethod
    def _snapshot_from_row(
        row: Mapping[str, Any], findings: list[dict[str, Any]]
    ) -> dict[str, Any]:
        return {
            "snapshot_id": row["id"],
            "vault_id": row["vault_id"],
            ...
            "sampled_findings": int(row["sampled_findings"]),
            "findings_truncated": bool(row["findings_truncated"]),
            "summary": _decode_object(
    ...
    def latest_snapshot(
        ...
        vault_id: int,
        *,
        finding_limit: int = MAX_HEALTH_PAGE_SIZE,
    ) -> dict[str, Any] | None:
        vault_id = _positive_or_zero(vault_id, name="vault_id")
        if vault_id == 0:
            raise ValueError("vault_id must be positive")
        ...
        finding_limit = min(int(finding_limit), MAX_HEALTH_PAGE_SIZE)
        row = self.connection.execute(
            """
            SELECT id, vault_id, catalog_revision, status, created_at, checked_at,
                   total_findings, sampled_findings, findings_truncated, summary_json,
                   uid, gid, error_code, error_message
            FROM filesystem_health_snapshots
            WHERE vault_id=%s
        ...
        if row is None:
            return None
        findings = self.list_findings(row["id"], limit=finding_limit)
        return self._snapshot_from_row(row, findings)
    ...
    def list_findings(
        ...
        snapshot_id: str,
        *,
        limit: int = MAX_HEALTH_PAGE_SIZE,
        offset: int = 0,
    ) -> list[dict[str, Any]]:
        identifier = _bounded_text(
            snapshot_id,
        ...
        limit = min(int(limit), MAX_HEALTH_PAGE_SIZE)
        rows = self.connection.execute(
            """
            SELECT id, snapshot_id, code, scope, path, message, remediation, occurrences
            FROM filesystem_health_findings
            WHERE snapshot_id=%s
            ORDER BY id
            LIMIT %s OFFSET %s
            """,
    ...
    def prune_snapshots(self, vault_id: int, *, keep: int = 10) -> int:
        vault_id = _positive_or_zero(vault_id, name="vault_id")
        if vault_id == 0:
            raise ValueError("vault_id must be positive")
        if keep < 1:
            raise ValueError("keep must be positive")
        keep = min(int(keep), 1000)
        result = self.connection.execute(
            """
            DELETE FROM filesystem_health_snapshots
            WHERE vault_id=%s
              AND id NOT IN (
                  SELECT id
                  FROM filesystem_health_snapshots
            ...
            """,
            (vault_id, vault_id, keep),
        )
        return max(int(result.rowcount), 0)


# Small function adapters make the persistence seam convenient for code that
# does not need to retain a repository object, while preserving the same
# caller-owned transaction rules.
...
def save_filesystem_health_snapshot(connection: Any, **kwargs: Any) -> dict[str, Any]:
    return FilesystemHealthStore(connection).save_snapshot(**kwargs)
