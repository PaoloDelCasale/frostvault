...
OIDC values are the exception to the generic runtime-setting store: they use the
validated draft/activation lifecycle in ADR-0010. Environment values are the
initial active fallback until the first lifecycle mutation; `OIDC_ENABLED`
explicitly controls whether that fallback is enabled.
